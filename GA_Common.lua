  ...
  for idx = 1, count do
    ...
    while word1 ~= word2 do
      if (word1 & 1) == (word2 & 1) then
        bits_common = bits_common + 1
      end
      word1 = word1 >> 1
      word2 = word2 >> 1
      bit_pos = bit_pos + 1
    end
    bits_common = bits_common + ((idx == count) and (bits % WORD_SIZE) or WORD_SIZE) - bit_pos
  end
...
function BinaryCrossoverUniform(words1, words2, bias)
  local count = (#words1 < #words2) and #words1 or #words2
  local bits = (words1.bits < words2.bits) and words1.bits or words2.bits
  local off1_words, off2_words = {bits = bits}, {bits = bits}
  for idx = 1, count do
    local word1 = words1[idx]
    local word2 = words2[idx]
    local off1_word, off2_word = 0, 0
    local bit_mask = 1
    local bits_count = (idx == count) and (bits % WORD_SIZE) or WORD_SIZE
    for bit = 1, bits_count do
      local allele1 = (word1 & bit_mask)
      local allele2 = (word2 & bit_mask)
      if FlipCoin(bias) then
        off1_word = off1_word + allele1
        off2_word = off2_word + allele2
      else
        off1_word = off1_word + allele2
        off2_word = off2_word + allele1
      end
      bit_mask = bit_mask << 1
    end
    table.insert(off1_words, off1_word)
    table.insert(off2_words, off2_word)
  end
  
  return off1_words, off2_words
end

function Binary1PointCrossover(words1, words2, xsite)
  ExchangeTailBits(words1, words2, xsite)
  return words1, words2
end

function BinaryMutation(words, bias)
  for i, word in ipairs(words) do
    local mask = 1
    local bits_count = (i == #words) and (words.bits % WORD_SIZE) or WORD_SIZE
    for bit = 1, bits_count do
      if FlipCoin(bias) then
        word = ((word & mask) ~= 0) and (word - mask) or (word + mask)
      end
      mask = mask << 1
    end
    words[i] = word
