...
local function GetFuncsMinMaxSizes(funcs, descr)
  ...
  for name, func_points in pairs(funcs) do
    ...
    for idx = start_idx, end_idx do
      ...
      if pt.stddev then
        local y_up = pt.y - pt.stddev
        local y_down = pt.y + pt.stddev
        if scale then
          y_up = scale(y_up)
          y_down = scale(y_down)
        end
      min_y = (y_up < min_y) and y_up or min_y
      max_y = (y_down > max_y) and y_down or max_y
      end
    end
  end
...
function DrawGraphs(bmp, funcs_data, descr)
  ...
  for _, name in ipairs(order) do
    ...
    local stddev_color = func_points.stddev_color or color
    ...
    for idx = start_idx, end_idx do
      ...
      if pt.stddev then
        local y1, y2 = pt.y - pt.stddev, pt.y + pt.stddev
        local y1 = math.floor(Oy - scale_y * ((scale and scale(y1) or y1) - center_y))
        local y2 = math.floor(Oy - scale_y * ((scale and scale(y2) or y2) - center_y))
        bmp:DrawLine(x, y1, x, y2, stddev_color)
        bmp:DrawLine(x - box_size, y1, x + box_size, y1, stddev_color)
        bmp:DrawLine(x - box_size, y2, x + box_size, y2, stddev_color)
      end
      if pt.text then
